...
<div class="pageHeader">
    <form rel="pagerForm" onsubmit="return navTabSearch(this);" action="<?=\Core\Lib::getUrl('CreditHkAmount','index')?>" method="post">
        <div class="searchBar">
...
<div class="pageContent">
    <div class="panelBar">
        <ul class="toolBar">
            ...
            <!--<li><a class="add" rel="reload" title="添加测试数据" onclick="addend()"><span>添加测试数据</span></a></li>
            <li><a class="add" rel="reload" title="添加测试数据" onclick="addbill()"><span>添加测试数据</span></a></li>
            <li><a class="delete" rel="reload" title="一键删除测试数据" onclick="delbill()"><span>一键删除测试数据</span></a></li>-->
        </ul>
    </div>
    <?php
    if(\Core\Lib::request('month')){
        ...
        <table class="list" width="100%" layoutH="90">
            ...
            <?php
            $sum = 0;
            //$data1 = array_unshift($data);
            foreach ($data['day'] as $k=>$v) {
                $sum +=  $v['total'];
            }
            ?>
            <?php
            for($j = 1;$j <= \Core\Lib::getMonthLastDay($year,\Core\Lib::request('month'));$j++){
                ?>
                <tr target="id" rel="<?=$j?>">
                    <td align="center"><a href=""><strong style="color:#ff0000;"><?=$j?>日</strong></a></td>
                    <td align="center"><?=\Core\Lib::formatMoney($data['day'][$j]['total'],2)?></td>
                    <?php
                    for($i = 0;$i <= 23;$i++){
                        ?>
                        <td align="center">
                            <?php
                            echo \Core\Lib::formatMoney($data['day'][$j][$i],2);
                            ?>
                        </td>
                        <?php
                    }
            ...
            <tr target="id" rel="<?=$j?>">
                ...
                <td align="center"><?=\Core\Lib::formatMoney($sum,2);?></td>
                <?php for($i = 0;$i <= 23;$i++){ ?>
                    <td align="center"><?php
                        echo \Core\Lib::formatMoney(array_sum($data['fday'][$i]),2);
                        ?></td>
                <?php }?>
            </tr>
        ...
        <div class="panelBar">
            <div class="pages">
                <span>合计：<?php echo \Core\Lib::formatMoney($sum,2); ?></span>
            </div>
        </div>
        <?php
    }else{
            ...
            <?php for($j = 1;$j <= 12;$j++){?>
                <tr target="id" rel="<?=$j?>">
                    ...
                    if(\Core\Lib::request('type') == 'week') {
                        ...
                        ?>
                        <td align="center">
                            <?=\Core\Lib::formatMoney($data['day'][$j][$i],2);?>
                        </td>
                    <?php } } ?>
                </tr>
            <?php }?>
        ...
        <div class="panelBar">
            <div class="pages">
                <span>合计：<?=\Core\Lib::formatMoney($sum,2);?></span>
            </div>
        </div>
        <?php
    }
...
<script>
    function addend(){
        $.post('<?php echo \Core\Lib::getUrl("ReportsYiBaoCA","append"); ?>',[],function(data){
            navTabPageBreak();
        })
    }
</script>
...
<script>
    function addbill(){
        $.post('<?php echo \Core\Lib::getUrl("ReportsYiBaoCA","appendbill"); ?>',[],function(data){
            navTabPageBreak();
        })
    }
    function delbill(){
        $.post('<?php echo \Core\Lib::getUrl("ReportsYiBaoCA","delbill"); ?>',[],function(data){
            navTabPageBreak();
        })
    }
